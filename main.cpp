...
Result performOp(Token op, string leftValue, string leftType, string rightValue, string rightType)
{
   ...
   else if (op.ID == TokenID::MINUS)
   {
	 ...
	 // if any of the values type is str 
		  cout << "**Error: type error with '" << op.Value << "' @ "<< "("<<op.Line<<", "<<op.Col  << "), expr is '" << leftType << "' + '"<< rightType <<"'"<<endl; 
		  result.Value = "0";
		  result.Type = "int";
	  }
   ...
   else if (op.ID == TokenID::DIV)
   {
	  ...
	  if (leftType == "str" || rightType == "str"){
         ...
         result.Value = "0";// NOTE: python checks for divide by 0 and throws an error,
                            // we're going to just silently evaluate to 0.
	  }
      else
         result.Value = to_string(lhs / rhs);

      result.Type = "int";// int and int
   }
   else if (op.ID == TokenID::MOD)
   {
      ...
      if (rhs == 0)
	   ...
	   //if any value type is str 
         ...
         result.Value = to_string(lhs % rhs);// int and int
   ...
   else if (op.ID == TokenID::POWER)
   {
      //if any value type is str
	   if (leftType == "str" || rightType == "str"){
		  ...
		  int lhs = stoi(leftValue);// int and int
		  int rhs = stoi(rightValue);
...
//This decides if the value in a string is a number or not 
//it returns 1 if it is an integer 
//it returns -1 if it is an string
...
//This converts any number which is stored in form of string into int 
Result stringtoint(Token program[], int PC, Variable memory[], int NumVars){
	...
	c = searchRAM(memory, NumVars, program[PC].Value);
	
	

	if (a == 1){
		A.Value = A.Value; // if A.value is an integer 
		A.Type = "int"; 
	}
	//if string 
	else if(c == -1){
	...
	//if string 
		...
		cout << "**Error: cannot convert "<< program[PC].Value <<" containing '"<< A.Value << "' @ "<< "("<<program[PC].Line<<", "<<program[PC].Col  << ") to 'int'"<<endl;
		A.Value = "0";
	}
	A.Type = "int";
	return A; 
...
//this converts any int to a string 
...
//this finds weather the following is int or string 
//this returns the type in value and makes the type as str
...
//this takes input 
Result takeinput(Token program[], int PC, Variable memory[], int NumVars, int a ){
	Result A; 
	string input; 
	
	if (a == 0){
		cout << program[PC].Value; //if input(parameter)
...
void executeStmt(Token program[], int& PC, Variable memory[], int& NumVars)
{
   ...
   if (program[PC].ID == TokenID::IDENTIFIER)
   {
      ...
      else
      {
		 ...
		 else if(program[PC].Value == "input"){
			PC++;
			PC++; 
			if(program[PC].ID ==TokenID::RIGHT_PAREN){
				int a = 1;
				result = takeinput(program, PC, memory, NumVars,a) ;
				PC++; 
			}
			else{
				int a = 0; 
...
// id else loop
//this handles the if else statement is python code 
void ifelse(Token program[], int& PC, Variable memory[], int& NumVars){
	Result code;
	code.Value = "0";
	while(true)
	 {
	   if(program[PC].ID==TokenID::IF_KEYW) {
		  PC++;
		  code = evaluateExpr(program,PC,memory,NumVars);
	   }
	   ...
	   else
	   {
		   ...
		   PC++;
		   while(program[PC].ID==TokenID::ELSE_KEYW || program[PC].ID==TokenID::ELIF_KEYW){
			   while(program[PC].ID!=TokenID::UNDENT)
			   {
				   PC++;
			   }
...
int main(int argc, char* argv[])
{
   ...
   while (program[PC].ID != TokenID::EOS){
      ...
      //PC++;
	   if(program[PC].ID==TokenID::IF_KEYW || program[PC].ID==TokenID::ELIF_KEYW || program[PC].ID==TokenID::ELSE_KEYW){
		    ifelse(program, PC, memory, NumVars);
	   }
	   else{
		    executeStmt(program, PC, memory, NumVars);
	   } 
